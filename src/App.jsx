...
export default function App() {
  ...
  const fetchAppData = async (churchId) => {
    ...
    if (studentsData) setStudents(studentsData.map(s => ({ ...s, group: s.class_name, parentsName: s.parents_name, parentsPhone: s.parents_phone, consecutiveAbsences: s.consecutive_absences })));
    ...
    // 1. 현재 화면에 띄워진 날짜(selectedAttDate)의 출석을 불러옴
  ...
  const handleCreateChurch = async (e) => {
    ...
    const { data: churchData, error: churchError } = await supabase.from('churches')
      .insert([{ 
        ...
        pastor_name: createChurchForm.pastorName // 추가됨
      }])
      .select().single();
  ...
  // ⭐ 선택된 날짜(selectedAttDate) 기준으로 출석 데이터 저장
  const handleAttendance = async (id, status) => {
    setAttendance(prev => ({ ...prev, [id]: status }));
    
    // 만약 현재 수정 중인 날짜가 이번 주일(대시보드 기준)이라면, 대시보드도 즉시 반영!
  ...
  const saveStudentInfo = async () => {
    const studentData = { church_id: currentUser.churchId, name: editStudentModal.student.name, birth: editStudentModal.student.birth, class_name: editStudentModal.student.group, grade: editStudentModal.student.grade, school: editStudentModal.student.school, phone: editStudentModal.student.phone, parents_name: editStudentModal.student.parentsName, parents_phone: editStudentModal.student.parentsPhone, prayer: editStudentModal.student.prayer, gender: editStudentModal.student.gender };
    if (editStudentModal.isNew) {
      const { data, error } = await supabase.from('students').insert([studentData]).select().single();
      if (!error && data) { setStudents([...students, { ...data, group: data.class_name, parentsName: data.parents_name, parentsPhone: data.parents_phone, consecutiveAbsences: data.consecutive_absences }]); showToast('신규 학생이 등록되었습니다.'); }
    } else {
      const { data, error } = await supabase.from('students').update(studentData).eq('id', editStudentModal.student.id).select().single();
      if (!error && data) { setStudents(students.map(s => s.id === data.id ? { ...data, group: data.class_name, parentsName: data.parents_name, parentsPhone: data.parents_phone, consecutiveAbsences: data.consecutive_absences } : s)); showToast('학생 정보가 업데이트되었습니다.'); }
    }
    closeEditStudent();
  };
  ...
  return (
    <div className="min-h-screen bg-stone-100 font-sans flex justify-center">
      <div className="w-full max-w-md bg-stone-50 min-h-screen relative shadow-2xl flex flex-col overflow-hidden">
        ...
        {editStudentModal.isOpen && editStudentModal.student && (<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200"><div className="flex justify-between items-center p-4 border-b border-stone-100 bg-white"><h3 className="font-bold text-stone-800 flex items-center text-lg"><UserCircle size={22} className="mr-2 text-emerald-500" />학생 상세 정보</h3><button onClick={closeEditStudent} className="text-stone-400 bg-stone-100 rounded-full p-1"><X size={20} /></button></div><div className="p-5 overflow-y-auto max-h-[70vh] space-y-4"><div className="grid grid-cols-2 gap-4"><div className="col-span-2 flex space-x-3"><div className="flex-1"><label className="text-xs font-bold text-stone-600 mb-1">이름</label><input type="text" name="name" value={editStudentModal.student.name} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm" /></div></div><div><label className="text-xs font-bold text-stone-600 mb-1">생년월일 (YYYY-MM-DD)</label><input type="text" name="birth" value={editStudentModal.student.birth} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm" /></div><div><label className="text-xs font-bold text-stone-600 mb-1">소속 반</label><select name="group" value={editStudentModal.student.group} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm">{visibleGroups.filter(g => g !== '전체').map(g => <option key={g} value={g}>{g}</option>)}</select></div><div><label className="text-xs font-bold text-stone-600 mb-1">학년</label><input type="text" name="grade" value={editStudentModal.student.grade} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm" /></div><div className="col-span-2"><label className="text-xs font-bold text-stone-600 mb-1">학교</label><input type="text" name="school" value={editStudentModal.student.school || ''} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm" /></div><div className="col-span-2"><label className="text-xs font-bold text-stone-600 mb-1">학생 연락처</label><input type="text" name="phone" value={editStudentModal.student.phone} onChange={handleEditChange} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-2 text-sm" /></div></div></div><div className="p-4 border-t border-stone-100 bg-white flex space-x-2"><button onClick={saveStudentInfo} className="flex-1 py-3 bg-emerald-500 text-white text-sm font-bold rounded-xl">저장</button></div></div></div>)}
        {editDutyModal.isOpen && editDutyModal.duty && (<div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200"><div className="flex justify-between items-center p-4 border-b border-stone-100 bg-white"><h3 className="font-bold text-stone-800 flex items-center text-sm"><Calendar size={18} className="mr-2 text-emerald-500" /> {editDutyModal.duty.date} 순서 변경</h3><button onClick={closeEditDuty} className="text-stone-400"><X size={16} /></button></div><div className="p-5 space-y-4"><div><label className="text-[11px] font-bold text-stone-500 mb-1">기도회 인도</label><select value={editDutyModal.duty.leader} onChange={(e) => setEditDutyModal(p => ({ isOpen: true, duty: { ...p.duty, leader: e.target.value } }))} className="w-full bg-stone-50 border border-stone-200 rounded-lg py-2.5 px-3 text-sm">{teachers.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}</select></div></div><div className="p-4 border-t border-stone-100 bg-[#FFFCF9] flex justify-end space-x-2"><button onClick={closeEditDuty} className="px-4 py-2 bg-white border border-stone-200 text-stone-600 text-xs font-bold rounded-lg">취소</button><button onClick={saveEditDuty} className="px-4 py-2 bg-emerald-500 text-white text-xs font-bold rounded-lg">저장</button></div></div></div>)}

        {confirmDialog.isOpen && (<div className="absolute inset-0 z-100 flex items-center justify-center bg-black/50 p-4"><div className="bg-white rounded-2xl p-6 shadow-2xl w-full max-w-xs text-center animate-in zoom-in-95 duration-200"><AlertCircle size={40} className="mx-auto text-emerald-500 mb-4" /><p className="text-stone-800 font-bold mb-6">{confirmDialog.message}</p><div className="flex space-x-3"><button onClick={closeConfirm} className="flex-1 bg-stone-100 py-3 rounded-xl font-bold">취소</button><button onClick={executeConfirm} className="flex-1 bg-emerald-500 text-white py-3 rounded-xl font-bold">확인</button></div></div></div>)}
