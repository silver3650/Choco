...
import { MOCK_STATS_WEEKLY } from '../mockData';
...
export default function AdminCenter({ 
  ...
  const displayGroups = Array.from(new Set([...uniqueGroups, ...addedGroups])).sort((a, b) => a.localeCompare(b, 'ko-KR'));

  const monthlyStatsMap = {};
  MOCK_STATS_WEEKLY.forEach(d => {
      const month = d.label.split(' ')[0];
      if (!monthlyStatsMap[month]) monthlyStatsMap[month] = { present: 0, total: 0, count: 0 };
      monthlyStatsMap[month].present += d.present;
      monthlyStatsMap[month].total += d.total;
      monthlyStatsMap[month].count += 1;
  });
  
  const computedMonthlyStats = Object.keys(monthlyStatsMap).map(m => ({
      label: m,
      present: Math.round(monthlyStatsMap[m].present / monthlyStatsMap[m].count),
      total: Math.round(monthlyStatsMap[m].total / monthlyStatsMap[m].count)
  }));

  const statsData = statPeriod === 'monthly' ? computedMonthlyStats : MOCK_STATS_WEEKLY;
  ...
  const handleAddGroup = async () => {
    ...
    if (displayGroups.includes(trimmedName)) { showToast("이미 존재하는 반입니다.", "error"); return; }
    ...
    if (error) { showToast("반 추가에 실패했습니다.", "error"); } 
      ...
      setNewGroupName('');
      if (batchGroup === '1반' && displayGroups.length === 1) setBatchGroup(trimmedName);
    }
  };
  ...
  const handleSaveTeacherInfo = async () => {
    ...
    const { error } = await supabase.from('teachers').update({ 
      ...
      birth: editingTeacher.birth, 
      role: editingTeacher.role, 
      class_name: editingTeacher.group 
  ...
  const handleDeleteTeacher = async (e, teacherId, teacherName) => {
    e.stopPropagation(); // 모달 창이 열리는 것을 방지
    const confirmDelete = window.confirm(`[경고] ${teacherName} 선생님을 정식 교사 명단에서 완전히 삭제하시겠습니까?`);
    if (!confirmDelete) return;

    const { error } = await supabase.from('teachers').delete().eq('id', teacherId);
    if (error) {
      showToast("교사 삭제 중 서버 오류가 발생했습니다.", "error");
    } else {
      showToast(`${teacherName} 선생님이 명단에서 삭제되었습니다.`);
  ...
  const handleRejectPendingTeacher = async (teacherId, teacherName) => {
    ...
    if (error) {
      ...
      showToast(`${teacherName} 선생님의 가입이 거절되었습니다.`);
      setPendingTeachers(pendingTeachers.filter(pt => pt.id !== teacherId));
    }
  };

  // ⭐ 교회 설정 저장 시 실제 DB 업데이트 연결
  const handleSaveChurchSettings = async () => {
    const { error } = await supabase.from('churches')
      .update({
        name: currentUser.churchName,
        dept: currentUser.deptName,
        pastor_name: currentUser.pastorName, // 추가된 사역자 데이터!
    ...
    if (error) {
      console.error(error);
      showToast("교회 설정 저장에 실패했습니다. (서버 오류)", "error");
    } else {
      showToast("교회 설정이 성공적으로 저장 및 반영되었습니다!");
    }
  };
    ...
    <div className="p-4 space-y-4">
      ...
      {adminView === 'settings' && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100 animate-in fade-in">
          ...
          <div className="space-y-4">
            ...
            {/* ⭐ 저장 함수(handleSaveChurchSettings) 연결 */}
      ...
      {adminView === 'stats' && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-stone-100">
          ...
          {statPeriod === 'custom' && (
            <div className="mb-4 p-3 bg-emerald-50/50 border border-emerald-100 rounded-xl flex items-center justify-between"><div className="flex items-center space-x-2 text-xs"><input type="date" className="bg-white border rounded p-1" defaultValue="2026-05-01" /><span>~</span><input type="date" className="bg-white border rounded p-1" defaultValue="2026-05-31" /></div><button className="bg-emerald-500 text-white text-[10px] font-bold px-3 py-1.5 rounded-lg">조회</button></div>
          )}
          <div className="mt-4 border border-stone-100 rounded-xl bg-[#FFFCF9] pt-4 overflow-hidden relative">
            ...
            <div className="overflow-x-auto hide-scrollbar w-full pb-2">
              <svg width={chartWidth} height={chartHeight} viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="min-w-max" style={{ width: chartWidth }}>
                ...
                {statsData.map((d, i) => {
                  const px = getX(i); const py = getY(d.present); const ay = getY(d.total - d.present); const pct = Math.round((d.present / d.total) * 100);
                  return (
                    <g key={i}>
                      <text x={px} y={chartHeight - 15} textAnchor="middle" fontSize="10" fill="#78716c" fontWeight="bold">{d.label}</text>
      ...
      {adminView === 'teachers' && (
        <div className="space-y-4">
           <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-100 mb-4">
             ...
             <div className="flex space-x-2"><input type="text" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} placeholder="예: 고3부, 4반" className="flex-1 bg-stone-50 border border-stone-200 rounded-lg px-3 py-2 text-sm" /><button onClick={handleAddGroup} className="bg-emerald-500 text-white px-4 rounded-lg text-xs font-bold">추가</button></div>
             <div className="flex flex-wrap gap-2 mt-3">{displayGroups.filter(g => g !== '전체').map(g => <span key={g} className="text-xs bg-stone-100 text-stone-600 px-2 py-1 rounded-md font-bold">{g}</span>)}</div>
           </div>

           <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-100">
             <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-stone-800 text-sm">학생 반 일괄 배정 관리</h3><select value={batchFilterGroup} onChange={(e) => setBatchFilterGroup(e.target.value)} className="text-xs font-bold bg-stone-50 border border-stone-200 rounded-lg p-1.5">{displayGroups.map(g => <option key={g} value={g}>{g} 보기</option>)}</select></div>
             {selectedStudentIds.length > 0 && (
               <div className="bg-emerald-50 p-3 rounded-lg border border-emerald-100 mb-3 flex items-center justify-between"><span className="text-xs font-bold text-emerald-700">{selectedStudentIds.length}명 선택됨</span><div className="flex space-x-2"><select value={batchGroup} onChange={(e) => setBatchGroup(e.target.value)} className="text-xs font-bold border rounded-md py-1 px-2">{displayGroups.filter(g => g !== '전체').map(g => <option key={g} value={g}>{g}</option>)}</select><button onClick={handleBatchGroupChange} className="bg-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-md">이동</button></div></div>
             )}
             <div className="border border-stone-100 rounded-lg overflow-hidden max-h-60 overflow-y-auto">
               {students.filter(s => batchFilterGroup === '전체' || s.group === batchFilterGroup).map(student => (
                 <div key={`batch-${student.id}`} onClick={() => toggleStudentSelection(student.id)} className={`flex items-center p-3 border-b border-stone-100 cursor-pointer ${selectedStudentIds.includes(student.id) ? 'bg-emerald-50/50' : 'hover:bg-[#FFFCF9]'}`}>
                   ...
                   <div className="flex-1"><p className="text-sm font-bold text-stone-800">{student.name}</p><p className="text-[10px] text-stone-500">{student.grade} • 현재: <span className="font-bold text-sky-600">{student.group}</span></p></div>
                 </div>
               ))}
             </div>
           </div>
              ...
              {pendingTeachers.map(pt => (
                <div key={pt.id} className="bg-white p-3 rounded-lg border border-amber-200 flex flex-col space-y-3 mb-3">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <select value={pt.group} onChange={(e) => handlePendingChange(pt.id, 'group', e.target.value)} className="flex-1 text-xs font-bold border rounded p-1.5">{displayGroups.filter(g => g !== '전체').map(g => <option key={g} value={g}>{g}</option>)}<option value="전체">전체</option></select>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 space-y-4">
              ...
              <div><label className="block text-xs font-bold text-stone-600 mb-1">담당 반 지정 (매칭)</label><select value={editingTeacher.group} onChange={(e) => setEditingTeacher({...editingTeacher, group: e.target.value})} className="w-full border rounded-lg p-2.5 text-sm">{displayGroups.map(g => <option key={g} value={g}>{g}</option>)}</select></div>
            </div>
