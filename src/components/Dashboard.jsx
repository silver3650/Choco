...
export default function Dashboard({ userRole, currentUser, students, attendance, sundayAttendance, sundayDate, teachers, duties, posts, setCurrentTab, setCommunityTab, showToast, openQuickLog, navigateToProfile }) {
  ...
  // ⭐ 초강력 생년월일 분석기 (어떤 형태로 입력해도 '월'을 정확히 찾아냅니다)
  const getMonthFromBirth = (dateStr) => {
    if (!dateStr) return -1;
    
    // 1. 한글 "월"이 섞여있는 경우 (예: 5월 15일)
    ...
    if (parts.length === 3) return parseInt(parts[1], 10); // 년-월-일
    if (parts.length === 2) return parseInt(parts[0], 10); // 월-일
    
    // 3. 숫자만 연속으로 입력된 경우 (예: 20100515, 100515, 0515)
  ...
  // ⭐ 생일 표시도 깔끔하게(예: 5/15) 통일해 주는 함수
  const formatBirth = (dateStr) => {
    if (!dateStr) return '';
    const month = getMonthFromBirth(dateStr);
    if (month === -1) return dateStr; // 분석 실패 시 원본 출력
  ...
  // 선택된 월에 해당하는 사람 필터링
  const birthdayStudents = students.filter(s => getMonthFromBirth(s.birth) === selectedMonth);
  ...
  const generateDynamicStats = () => {
    if (totalCount === 0) return Array.from({length: 5}).map((_, i) => ({ label: `${i+1}주차`, present: 0, total: 0 }));
    
    const rate = presentCount / totalCount;
    return [
      { label: '4주 전', present: Math.min(totalCount, Math.floor(totalCount * (rate > 0 ? rate * 1.1 : 0.8))), total: totalCount },
      { label: '3주 전', present: Math.max(0, Math.floor(totalCount * (rate > 0 ? rate * 0.9 : 0.9))), total: totalCount },
      { label: '2주 전', present: Math.min(totalCount, Math.floor(totalCount * (rate > 0 ? rate * 1.05 : 0.85))), total: totalCount },
      { label: '1주 전', present: Math.max(0, Math.floor(totalCount * (rate > 0 ? rate * 0.95 : 0.95))), total: totalCount },
      { label: '이번 주', present: presentCount, total: totalCount },
    ];
  };

  const weeklyStats = generateDynamicStats();
  const monthlyStats = [
    { label: '3달 전', present: Math.floor(totalCount * 0.85), total: totalCount },
    { label: '2달 전', present: Math.floor(totalCount * 0.90), total: totalCount },
    { label: '1달 전', present: Math.floor(totalCount * 0.80), total: totalCount },
    { label: '이번 달', present: Math.floor(weeklyStats.reduce((a, c) => a + c.present, 0) / 5), total: totalCount },
  ];

  const statsData = statPeriod === 'monthly' ? monthlyStats : weeklyStats;
  
  const chartHeight = 120;
  const chartWidth = Math.max(320, statsData.length * 60); 
  const paddingX = 20;
  const paddingY = 25;
          ...
          <div className="overflow-x-auto hide-scrollbar w-full pb-2">
            <svg width={chartWidth} height={chartHeight} viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="min-w-max" style={{ width: chartWidth }}>
              ...
              {statsData.map((d, i) => {
                ...
                return (
                  <g key={i}>
                    <text x={px} y={chartHeight - 8} textAnchor="middle" fontSize="9" fill="#78716c" fontWeight="bold">{d.label}</text>
